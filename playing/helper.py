...
def plotting_training_log(num_episode, plotted_data, successes, failures, goal, distance, agent_init_pos):
...
def trajectory_plots(plotted_data, i):
    ...
    plt.xlabel("Episodes")
    plt.ylabel("Successful/Failed Trajectories and Ratio")
    plt.savefig("failed_success_ratio" + str(i) + ".png")
    ...
    plt.xlabel("Episodes")
    plt.ylabel("Successful Trajectories")
    plt.savefig("successful" + str(i) + ".png")
    ...
    plotted_data.plot(kind="bar", x="Episodes", y=["Step_num"], legend=True, fontsize=12,
                      title="Number of step toward goal per Episode")
    plt.xlabel("Episodes")
    plt.ylabel("Step_num")
    plt.savefig("Step_num" + str(i) + ".png")
