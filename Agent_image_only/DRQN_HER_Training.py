...
epsilon_max = 1
epsilon_min = 0.05
input_size = 518  ## size of the input to the LSTM
...
model = DRQN(action_n=action_n, cell=cell, fcl_dims=fcl_dims, scope="model",
             save_path=dir, nodes_num=nodes_num, input_size=input_size, images_only=images_only)
    ...
    for n in range(start, num_episodes):
        ...
        while not done:
            ...
            episode_buffer.add(np.reshape(
                np.array([pre_action_idx, features, pos_state, curr_action_idx, reward, features_, pos_state, done, goal]),
                [1, 9]))
