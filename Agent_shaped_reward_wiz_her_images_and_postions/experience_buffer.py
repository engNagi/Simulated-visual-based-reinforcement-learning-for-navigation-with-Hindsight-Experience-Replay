...
class experience_buffer(object):
    ...
    def her_shaped(self):
        ...
        for k in range(self.her_samples):
            ...
            goal_state = her_traj[-1][4]  ##pre_Action, state, action, reward, next_state, done, goal
            goal_pos = goal_state[-3:]
            for trans in range(len(her_traj)):
                pre_action = her_traj[trans][0]
                state = her_traj[trans][1]
                pre_state_pos = state[-3:]
                action = her_traj[trans][2]
                next_state = her_traj[trans][4]
                ...
                her_traj[trans][2] = action
                her_traj[trans][3] = reward
                her_traj[trans][4] = next_state
                her_traj[trans][5] = done
                her_traj[trans][6] = goal_pos
